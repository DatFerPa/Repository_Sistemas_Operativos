...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (registerIR_CPU.operationCode) {
		...
		//Instruccion MEMADD
		case 'm':
			//buscar el valor que se encuntra en la direccion de memoria del operando 2
			//definimos la celda de memoria a buscar en el mar
			registerMAR_CPU=registerIR_CPU.operand2;
			//le decimos al los buses que pasen la informacion de la CPU a la memoria
			Buses_write_AddressBus_From_To(CPU, MAINMEMORY);
			//decimos que se lea de memoria principal
			MainMemory_readMemory();
			//sumamos el valor del operando1, con el valor que s eencontraba en la direccion de memoria
			//del operando2
